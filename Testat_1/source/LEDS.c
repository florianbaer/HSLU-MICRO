...
void LEDTurnOn(uint32_t led, led_counter* _counter){
	...
	case CONST_LED_F_L_RED_LED:
		GPIO_PinOnWithCounter(LED_LED_RED_FL_GPIO, LED_LED_RED_FL_PIN, &(_counter->FLR));
		break;
	case CONST_LED_F_L_GREEN_LED:
		GPIO_PinOnWithCounter(LED_LED_GREEN_FL_GPIO, LED_LED_GREEN_FL_PIN, &(_counter->FLG));
		break;
	case CONST_LED_F_L_BLUE_LED:
		GPIO_PinOnWithCounter(LED_LED_BLUE_FL_GPIO, LED_LED_BLUE_FL_PIN, &(_counter->FLB));
		break;
	case CONST_LED_F_R_RED_LED:
		GPIO_PinOnWithCounter(LED_LED_RED_FR_GPIO, LED_LED_RED_FR_PIN, &(_counter->FRR));
		break;
	case CONST_LED_F_R_GREEN_LED:
		GPIO_PinOnWithCounter(LED_LED_GREEN_FR_GPIO, LED_LED_GREEN_FR_PIN, &(_counter->FRG));
		break;
	case CONST_LED_F_R_BLUE_LED:
		GPIO_PinOnWithCounter(LED_LED_BLUE_FR_GPIO, LED_LED_BLUE_FR_PIN, &(_counter->FRB));
		break;
	case CONST_LED_R_L_RED_LED:
		GPIO_PinOnWithCounter(LED_LED_RED_RL_GPIO, LED_LED_RED_RL_PIN, &(_counter->RLR));
		break;
	case CONST_LED_R_R_RED_LED:
		GPIO_PinOnWithCounter(LED_LED_RED_RR_GPIO, LED_LED_RED_RR_PIN, &(_counter->RRR));
		break;
	default:
		break;
	}
