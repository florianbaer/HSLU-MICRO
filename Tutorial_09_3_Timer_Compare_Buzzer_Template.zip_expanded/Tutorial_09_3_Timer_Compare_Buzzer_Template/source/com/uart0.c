...
static volatile uint16_t rxBufCount = 0;
static uint16_t rxBufWritePos = 0;
static uint16_t rxBufReadPos = 0;

/**
 * the transmit queue of this driver, implemented as ring buffer
 */
static char txBuf[UART0_TX_BUF_SIZE];
static volatile uint16_t txBufCount = 0;
static uint16_t txBufWritePos = 0;
static uint16_t txBufReadPos = 0;
...
void UART0_RX_TX_IRQHandler(void)
{
  ...
  // since buffer is always expected to be 1 of size,
  // always flush it after reading. Otherwise
  // the RXFIFO gets confused
